...
5. 预渲染；
   ...
   prerender-spa-plugin vue.config.js配置
     使用 plugins:[
       new PrerenderSPAPlugin({
         staticDir: path.join(__dirname, 'dist'),
         routes: ['/','/about','/home'], // 需要预渲染的路由
   ...
   修改title描述关键字 vue-meta-info
     使用到页面组件中配置
     <script>
       export default {
         name: 'Home',
         metaInfo: { // 使用 vue-meta-info 插件 配置
           title: '自定义title',
           meta: [
             { name: 'keywords', content: '自定义keywords' },
             { name: 'description', content: '自定义description' }
           ]
         }
       }
     </script>
   可以解决：
      1.打包多个页面
      2.可以为每个单独生成的页面添加title描述关键字
      3.接口数据是在html生成之前就放在页面上的，爬虫可以抓取到内容
   存在问题：
     1.预渲染无法配置动态路由（例如：详情页根据id展示）
   适合做的项目：
     1.一个项目可能某几个页面需要做SEO
     2.如果title描述关键字是来自接口数据，配置meta-info也是不行的
...
6. 服务端渲染ssr;
   react : next
     ...
     解决seo问题
      1. 前后端不分离（前端负责写静态页后端负责填充模板）
          2.压力在后端
            好处：安全，不用对外暴漏接口
       
      2.前后端分离
         spa单页面应用vue-cli本身处理不了seo
           压力在客户端
             好处：开发速度快，前后端分离，可以复用组件
         预渲染
           压力在客户端
           一个项目不是所有页面都做seo
           问题：在html页面加载之前数据过来渲染之后才有html的dom结构，这样的话可能也会存在页面空白的情况  
         服务端渲染 
           压力在客户端
           问题：1.起两个服务，【一个是后端自己语言的服务，一个是nodejs的服务】
...
#### 27、为什么需要虚拟DOM？
- 1.框架设计
- - vue设计最低柯里化是组件更新，如果一个组件里有几千个dom元素，而实际更新的只有一个，这样性能开销就太大了
- 2.解耦运行环境
#### 28、一个接口多次连续请求，保证请求的是最后一次请求的响应结果
- 取消上次请求
- - xhr使用 xhr.abort()方法
- - fetch使用 AbortController 对象可以用来取消 fetch 请求，通过调用其 abort 方法来取消请求
- - axios使用 axios.CancelToken 对象可以用来取消 axios 请求，通过调用其 cancel 方法来取消请求
#### 29、有多个IP地址，如何在最短时间内找出RTT（Round-Trip-Time 从请求到收到响应的时间）最小的IP地址（并发数量最大为10）？
- 使用最大并发数分批次请求接口，先用第一批发送请求哪个ip最先响应终止其他ip的响应，将最短时间的ip并记录最短时间，换另一批接着请求，
- 判断有没比上次最短时间还短的请求，如果全大于最短时间就终止响应节省时间， 有就更新记录继续比较，直到请求完就可以获取最短时间的请求。
#### 30、ajax进度监控
- 1.fetch做不了请求进度监控，xhr（axios）可以
- ```javascript
  // xhr
  const xhr = new XMLHttpRequest();
  xhr.upload.addEventListener('progress', function (event) {
    console.log(event.loaded, event.total); // 已加载大小 总数
  })
  ...
  instance.interceptors.request.use(config => {
    config.onUploadProgress = function (progressEvent) {
      console.log(progressEvent.loaded, progressEvent.total) // 已加载大小 总数
    }
    return config
  })
  ```
- 2.fetch和xhr(axios)都可以监控响应进度
- ```javascript
  // xhr
  const xhr = new XMLHttpRequest();
  ...
  instance.interceptors.request.use(config => {
    config.onDownloadProgress = function (event) {
      console.log(event.loaded, event.total); // 已加载大小 总数
    }
    return config
  })
  ...
  export function request(options={}){
   const {url, method='GET', data={}, headers={}, onProgress} = options;
...
#### 32、使用元组生成联合类型
```typescript
const value1 = ['a', 'b', 'c'] as const;
const value2 = [1,2,3,4] as const;
type Value1 = (typeof value1)[number]
type Value2 = (typeof value2)[number]

function foo(x: Value1 ,y: Value2) {}
````
...
```
- 空间复杂度：算法在计算过程中所消耗的**额外的空间
- 注意：只有因算法开辟的空间，才能计算空间复杂度。
- 
#### 4、栈的介绍
...
- - - 原理：黑客诱导用户点击恶意链接，在用户不知情的情况下，以用户身份发送请求。
- - - 预防：
- - - 1.严格跨域请求限制，验证referer
- - - 2.为cookie设置Samesite属性(strict)
- - - 3.关键接口使用短信验证码等双重验证
- - 9.点击劫持（clickjacking）
- - - 原理：黑客在自己的网站，使用隐藏的\<iframe>嵌入其他网页，诱导用户按照顺序点击。
- - - 预防：使用JS预防，
- - - ```js
        if(top.localtion.hostname !== window.location.hostname){
          alert("检测到您正在访问不安全的网站，即将跳到安全页面")
          top.location.href = window.location.href
        }
        ```
- - - 使用X-Frame-Options HTTP响应头，设置DENY或SAMEORIGIN，让iframe只能加载同域名的网页。      
- - 10.使用内容安全策略（CSP）来防止XSS（Cross-Site Scripting，跨站脚本攻击）
- - - 1.设置HTTP头中的Content-Security-Policy
- - - 2.使用meta标签
- - - ```html
...
- - - 现代框架默认会屏蔽XSS攻击，例如React和Vue.js。除非自己手动开启
- - - - Vue v-html
- - - - React dangerouslySetInnerHTML
- - - SQL注入
- - - - 普通登录方式，输入用户名和密码，然后服务端去数据库查询数据
- - - - ```sql
         SELECT * FROM users WHERE username = 'admin' AND password = '123456'
        ```
- - - - 如果用户输入用户名：'delete from users where 1=1; --'，密码：123456，那么SQL语句就会变成：
- - - - ```sql
         SELECT * FROM users WHERE username = ''delete from users where 1=1; --'' AND password = '123456'
        ```
- - - - 这样就会删除所有用户数据。
- - - - 防止SQL注入的方法：
- - - - - 1.服务端进行特殊字符转换，如把' 转换为\\'
- - 11.DDOS攻击(Distributed Denial of Service 分布式拒绝服务) 
- - - 通过大规模的网络流量淹没目标服务器或周边基础设施，以破坏目标服务器、服务或网络正常流量的恶意行为（类似于恶意堵车，妨碍正常车辆通行） 
- - - 预防：软件层面不好做，可以选择商用防火墙如阿里云 WAF、腾讯云 WAF、华为云 WAF 等，硬件层面可以购买防火墙设备，如华为 USG、深信服防火墙等。

- ### 16、跨域请求
- - 1.浏览器同源策略：同源策略是一种安全协议，是指协议、域名、端口都相同，同源策略是一种安全协议，指一段脚本只能读取来自同一来源的窗口和文档的属性。
- - 2.同源策略一般限制 ajax 网络请求，不能跨域请求server
- - - 跨域请求的解决方案：
- - - - 1.通过jsonp跨域，缺点是只支持get方法，不安全
- - - - - ```js
          // 封装jsonp函数
          ...
          // 使用方法
          jsonp('http://localhost:3000', { name: 'jack' }, function (res) {
            console.log(res)
          })   
          ```
- - - - 2.通过修改document.domain来跨子域, 缺点是只能用于二级域名相同的情况，比如a.test.com和b.test.com
- - - - 3.通过window.name来进行跨域, 缺点是window.name的值大小是有限的，而且如果需要跨域的话，服务器端也需要做处理
- - - - 4.通过html5的postMessage来实现跨域请求，缺点是兼容性不是很好
- - - - 5.通过cors（跨域资源共享）来实现跨域请求，缺点是需要服务器端支持
- - - - 6.通过nginx反向代理来跨域，缺点是需要服务器端支持
- - 3.不会限制 img、script、iframe、link 等标签的跨域请求，这些标签可以跨域请求资源，但不会携带cookie信息 

- ### 17、webpack 常用plugin
- - 1.uglifyjs-webpack-plugin：压缩js代码
- - 2.html-webpack-plugin：基于HTML模板，生成对应的文件和引用关系，构建运行时的产物。
- - 3.extract-text-webpack-plugin：提取css文件
- - 4.mini-css-extract-plugin：提取css文件
- - 5.clean-webpack-plugin：清理构建目录
- - 6.hard-source-webpack-plugin：缓存模块，提高构建速度
- - 7.copy-webpack-plugin：复制文件
- - 8.webpack-bundle-analyzer：分析打包文件大小
- - 9.friendly-errors-webpack-plugin：友好的错误提示
- - 10.speed-measure-webpack-plugin：分析构建速度
- - 11.define-plugin：定义全局变量
- - 12.hot-module-replacement-plugin：热模块替换
- - 13.css-minimizer-webpack-plugin：压缩css代码
- - 14.terser-webpack-plugin：压缩js代码

- ### 18、loader和plugin的区别
- - loader：
- - - 转换器，核心是解析
- - - 用于对模块的源代码进行转换，loader可以在import或require时预处理文件，比如babel-loader、css-loader、style-loader等。
- - - webpack没有loader的话，无法处理非js文件，比如css、图片等，需要通过loader来处理这些文件，只能打包基础的cjs的js文件。
- - plugin：
- - - 扩展器，核心是扩展
- - - 在webpack的运行周期里，会有一些hooks对外暴露，所以在webapck打包编译的过程中，plugin会根据这些hook执行一些自定义的操作，来实现
      对输出结果的干预的增强。
- - 区别：
- - - loader 更专注于 文件的转换，是转换器，让webpack 处理非JS模块，一般在固定的流程（打包文件之前）起作用。
- - - plugin 更专注于 流程的扩展，是扩展器，让输出资源的能力更丰富，在整个生命周期里，都起作用。
