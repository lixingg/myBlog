...
> 1、核心定位
```text
uni-app 是DCloud出品的跨端前端框架
一套代码可以编译生成：
微信、支付宝、百度头条小程序
H5网页 安卓 APP IOS APP 快应用
真正实现一端开发 多端运行
```
> 2、技术栈基础
```text
语法完全兼容vue2 vue3 写法都支持
页面结构沿用template script style
组件 生命周期 数据渲染和vue高度一致
前端上手成本极低
```
>3、核心优势
```text
...
打包编辑时条件编译区分不同端
自动转换成对应平台的专属代码
小程序转换wxml/wxss APP渲染成原生视图
H5编译成网页代码
```
> 5、日常开发重点
```text
多用uni自带组件 少用HTML原生标签 保证多端兼容
巧用ifdef条件编译 处理各平台差异化逻辑
全局状态多用 uni.globalData
复杂场景搭配 Pinia/Vuex
小程序分包 APP热更新版本更新都是项目常用实战点
```
> 6、总结
```text
uni-app 是基于VUE开发的跨端框架，支持一套代码编译出小程序、H5、移动端APP
语法贴合VUE上手快内置大量原生设备能力，通过条件编译处理平台差异，大幅度提升多端项目开发效率
主要用它开发商城类小程序和移动端项目
```
